...
#  Extract the ending year from the 'Season' column (e.g., "2023-24" becomes "2024") and convert to integer
...
#  Step 2: Merge the MVP and Champions DataFrames on the 'Year' column
merged_df = pd.merge(mvp_df, champions_df, on='Year', how='inner')

# Step 3: Clean and format the merged data
#  Drop unnecessary columns
columns_to_drop = ['Lg', 'Voting', 'League', 'Points Leader', 'Rebounds Leader', 'Assists Leader', 'Win Shares Leader']
merged_df.drop(columns=columns_to_drop, inplace=True)

# Clean player names and finals MVP names using `unidecode` to remove special characters
...
#  Rename 'Player' to 'Season MVP' and format names and
...
# Rename 'Tm' to 'Team' and convert abbreviations to full team names
...
# Convert columns 'FG%', '3P%', and 'FT%' to percentages
